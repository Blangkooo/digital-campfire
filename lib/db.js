...
const AUTO_POOL = [
  ...
  { text: "We were supposed to break up that night. Instead we drove to the coast and watched the sun rise. That was twelve years ago.", category: "Story" },
...
async function addMessage(text, category) {
  await initDB();
  ...
  return { id, alias, created_at };
}

// dailyReset: runs once per calendar day.
// Clears ALL messages and injects 5 fresh auto ones so the board feels new each day.
async function dailyReset() {
  await initDB();
  const today = new Date().toISOString().slice(0, 10);

  const { rows: logged } = await pool().query('SELECT date FROM daily_log WHERE date=$1', [today]);
  if (logged.length > 0) return 0; // already done today

  // Wipe everything from previous day(s)
  await pool().query('DELETE FROM messages');

  // Pick 5 random messages from the pool (shuffle, take first 5)
...
module.exports = { getMessages, addMessage, dailyReset, initDB };
